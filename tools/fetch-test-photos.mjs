 ...
 * public/test-photos/, which is checked into git — the app's quick-load
 * buttons serve them from there in every build, dev or deployed, without
 * needing this script to run first. Re-run with --force to refresh a photo,
 * or add a new entry to PHOTOS and run once to fetch just that one.
 *
 * Licenses: CC BY 2.0 requires attribution — ATTRIBUTION.md (regenerated by
 * this script) ships alongside the photos in the repo and the built site.
 */

/**
 * Garment-upload extraction worker (Phase A4 — see
 ...
 * needed for the upload flow, so it's created (and its models downloaded)
 ...
 * person too; the parse is what lets pipeline/garmentExtract.ts strip the
 * wearer (head, arms, legs, non-target clothing) and keep just the
 * garment. Flat-lay photos (no person detected) skip extraction entirely
 * and use the plain matting result — see garmentExtract.ts for the
 * decision logic.
 */
import { env, pipeline, RawImage } from '@huggingface/transformers';
import { config } from '../config';
import { extractGarmentAlpha, type LabelMask } from '../pipeline/garmentExtract';
import type {
  MattingAccelerator,
...
async function process(bitmap: ImageBitmap, seq: number): Promise<void> {
  try {
    ...
    const extraction = extractGarmentAlpha(foregroundAlpha, labelMasks, w, h, config.garmentExtract);

 ...
 */
import type { RelightingConfig } from './config.js';
import { renderFeatheredMask } from './maskRender.js';
import type { DepthMapSource } from './types.js';
...
export function estimateLight(
  ...
  const { gradientGain, frontalBias, minIntensity, maxIntensity } = config;
...
export function applyGarmentShading(
  ...
  const { ambient, minShade, maxShade, aoGain, aoMax } = config;
